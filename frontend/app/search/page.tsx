...
const SearchPage = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const resp = await searchTestsByName(testName as string, testStatus ? testStatus as string : undefined);
    ...
    const columns = useMemo<MRT_ColumnDef<Test>[]>(
        () => [
            ...
            },
            {
                ...
                header: 'Status',
                Cell: ({ cell }) => {
                    return <Box sx={{width: '100px'}}><TestStatusLabel status={cell.getValue() as typeof TestStatus} /></Box>;
                },
            },
            {
                ...
                filterFn: (row, id, filterValue) => {
                    ...
                    const formatted = dayjs(row.original.startedAt as string).format('DD.MM.YYYY').toString();
            ...
            },
            {
                ...
                filterFn: (row, id, filterValue) => {
                    ...
                    const formatted = dayjs(row.original.stoppedAt as string).format('DD.MM.YYYY').toString();
                    return formatted.includes(filterValueModif);
                },
            },
            {
                Cell: ({cell}) => formatDuration(cell.row.original.startedAt as string, cell.getValue<string>()),
                accessorKey: 'stoppedAt', header: 'Duration', id: "1",
                filterFn: (row, id, filterValue) => {
                    const formattedDuration = formatDuration(row.original.startedAt as string, row.original.stoppedAt);
  ...
  return (
      <Box sx={{display: "flex", flexDirection: "column"}}>
          ...
          <Box sx={{display: "flex"}}>
              ...
              >
                  <Box sx={{display: 'flex', flexDirection: 'column', width: '90%', gap: '10px'}}>
                      <Box sx={{display: 'flex', flexDirection: 'column'}}>
                      <form onSubmit={async (e) => await handleSubmit(e)}>
                          ...
                          <Button sx={{width: '100%'}} type="submit" variant="outlined" color="primary">
                              Submit
                          </Button>
                          </Box>
                      </form>
